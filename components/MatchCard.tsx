...
export const MatchCard: React.FC<MatchCardProps> = ({ match, userId, onScoreChange, onViewDetails, specialTeams, specialPhases, scoringRules, isAdmin = false }) => {
  ...
  // Get current user's bet - PRIORITIZE DIRECT PROPERTIES SET IN APP.TSX
  // This ensures robust display even if user mapping logic fails
  const userScoreA = match.userScoreA ?? match.bets?.[userId]?.scoreA;
  const userScoreB = match.userScoreB ?? match.bets?.[userId]?.scoreB;
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border p-4 shadow-sm transition-all duration-300 flex flex-col justify-between h-full ${
      ...
      {/* Teams & Inputs - Adjusted with min-w-0 for flex children to prevent overflow */}
      <div className="flex items-center justify-between gap-3 relative z-20 mb-2">
        ...
        <div className="flex-1 flex flex-col items-center gap-2 group min-w-0">
          ...
          <span 
            title={match.teamA}
            className={`text-[10px] sm:text-xs font-bold text-center w-full whitespace-normal break-words leading-tight px-0.5 min-h-[2.5em] flex items-center justify-center ${specialTeams.includes(match.teamA) && !isLocked ? 'text-green-700 dark:text-yellow-400 uppercase tracking-wide' : 'text-slate-700 dark:text-slate-200'}`}
          >
            {match.teamA}
          </span>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center gap-2 group min-w-0">
          ...
          <span 
            title={match.teamB}
            className={`text-[10px] sm:text-xs font-bold text-center w-full whitespace-normal break-words leading-tight px-0.5 min-h-[2.5em] flex items-center justify-center ${specialTeams.includes(match.teamB) && !isLocked ? 'text-green-700 dark:text-yellow-400 uppercase tracking-wide' : 'text-slate-700 dark:text-slate-200'}`}
          >
            {match.teamB}
          </span>
        </div>
